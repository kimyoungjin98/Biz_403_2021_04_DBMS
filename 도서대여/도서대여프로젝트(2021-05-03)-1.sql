...
-- tbl_buyer 데이터 1개(1명의) 회원이
-- 다수의 tbl_book_rent table에 포함이 된다
-- tbl_buyer 1 : tbl_book_rent N = 1:다의 관계
-- N의 table에서 FK 설정을 하고, 1의 table과 관계를 맺는다

ALTER TABLE tbl_book_rent -- N table
...
mySQL경우 칼럼 속성에 AUTO_INCREMENT로 설정을 하면
...
-- FK 관계가 설정되지 않은 TABLE 간에는 조회되는 데이터가 실제와 다를 수 있다
...
-- 대여일이 2021년 4월 25일 이전이고 아직 미반납인 데이터
SELECT * FROM view_도서대여정보
WHERE 대여일 < '2021-04-25' AND 반납일 IS NULL;
...
VALUES(seq_book_rent.NEXTVAL, '2021-04-01', 'B0011', '9791188850341');

INSERT INTO tbl_book_rent(br_seq, br_sdate, br_bcode, br_isbn)
VALUES(seq_book_rent.NEXTVAL, '2021-04-01', 'B0011', '9791162540480');

-- 대여일이 2021-04-25 이전이고 아직 미반납된
-- 대여정보를 보여라
-- 그리고 회원코드순으로 보여라
SELECT 대여일, 회원코드, 회원명, 회원연락처, 도서명
FROM view_도서대여정보
