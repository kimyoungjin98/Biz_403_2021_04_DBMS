-- IOUSER 권한으로 접속
-- IOUSER는 DBA 권한을 갖고있기 때문에
-- 일반적인 표준 DDL, DML, 일부 DCL 명령등을 사용할 수 있다
-- DDL(Data Definition Lang. 데이터 정의어)
--      객체 생성 CREATE TABLE, INDEX, VIEW 
...
-- 데이터가 INSERT 되어버렸다
-- 나중에 확인을 해 보니 상품이 없어서 데이터 활용 가치가 
-- 매우 떨어지는 현상이 발생했다
-- 이런 상황을 "INSERT(추가, 삽입) 이상현상"이 발생했다
--      무결성이 훼손되었다
-- 기존의 Talbe을 제거하고 무결성을 유지하기 위한 제약조건을 
...
-- 2. projetion으로 설정된 칼럼들 데이터만 추출하여
-- 3. 리스트를 출력한다
...
-- tbl_iolist에 저장되어있는 데이터 리스트 중에서 
