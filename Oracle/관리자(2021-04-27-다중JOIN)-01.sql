...
(
    SELECT ST.st_num 학번, ST.st_name 학생이름, ST.st_dcode 학과코드,
            DP.dp_name 학과명, dp.dp_prof 담당교수, 
            ST.st_grade 학년, ST.st_tel 전화번호, ST.st_addr 주소
     FROM tbl_student ST
...
SELECT SC.sc_num 학번, ST.st_name 이름, ST.st_dcode 학과코드, ST.st_tel 전화번호,
        SC.sc_kor 국어, SC.sc_eng 영어, SC.sc_math 수학, DP.dp_name 학과명,
        DP.dp_prof 담당교수,
...
-- 각 그룹의 총점과 평균을 계산
SELECT 학과코드, 학과명 , SUM(총점) AS 학과총점, ROUND(AVG(평균),0) AS 학과평균
