...
-- 학번으로 묶어서 동일한 학번이, 동일한 학번의 국어점수의 합계 계산할때
...
SELECT SC.sc_num, ST.st_name, 
    ST.st_dept, SC.sc_kor, 
    SC.sc_eng, SC.sc_math 
...
-- 성적데이터는 모두 100개가 있지만 
-- 학생데이터에는 79개만 남아 있기 때문에
-- JOIN을 한 결과가 학생데이터와 같은 수인 79개만 
-- 조회(인출)되고 있다
...
-- 무너진 상태가 된다 == 참조 무결성이 오류가 발생했다 라고 표현한다
-- 참조 무결성이 문제가 생긴경우 
-- JOIN을 했을때 인출되는 데이터의 신뢰성을 보증 할수없다
SELECT * FROM tbl_score;
SELECT * FROM tbl_student;
...
--      같이 보여달라
-- 보통 table의 참조무결성 보증을 설정하는 경우가있는데 
-- 참조관계에 없는 다수의 테이블을 JOIN하여 보고 싶을때는
-- LEFT JOIN을 사용한다
-- 참조 무결성이 보증이 된 경우도 LEFT JOIN을 수행하면 
...
-- 학생 데이터와 성적 데이터간의 참조 무결성에 오류가 있기 때문에
