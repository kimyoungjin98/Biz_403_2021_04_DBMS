...
INSERT ALL
INTO tbl_books (bk_isbn, bk_pub, bk_name, bk_author, bk_trans, bk_date, bk_page, bk_price)
...
INTO tbl_books (bk_isbn, bk_pub, bk_name, bk_author, bk_date, bk_page, bk_price)
...
저자 항목(칼럼)을 보면 저자가 2명 이상인 데이터 있고
...
한 칼럼에 2개 이상의 데이터가 구분자(,)로 나뉘어 자장되는 것을
    막는 조치
...
-- 저자정보에는 ISBN과 저자명을 포함한 데이터를 추가해 주면 된다

SELECT ba_author, bk_title, ba_isbn
FROM tbl_books
    LEFT JOIN tbl_book_author
        ON bk_isbn = ba_isbn;
...
-- 9791162540572 이 도서에 저자를 추가하고 싶을때
-- 테이블의 ba_seq 칼럼에는 이미 등록된 값이 아닌 
-- 새로운 숫자를 사용하여 데이터를 추가해야한다
