...
SELECT bk_title AS 도서명, bk_price AS 가격
FROM tbl_books
WHERE is_price >= 20000;
...
-- tbl_books, tbl_author, tbl_company 세개의 table JOIN 하여 
-- ISBN, 도서명, 출판사명, 출판사대표, 저자, 저자연락처 로 출력되도록 SQL 작성

SELECT B.bk_isbn AS ISBN, B.bk_title AS 도서명, 
        c.cp_name AS 출판사명, c.cp_ceo AS 출판사대표, a.au_name AS 저자
        , a.au_tel AS 저자연락처
FROM tbl_books B
