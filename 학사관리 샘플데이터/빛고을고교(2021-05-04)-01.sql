...
AS
(
SELECT SC.sc_seq 일련번호 , SC.sc_stnum 학번,
        ST.st_name 학생이름, ST.st_tel 전화번호,
        SC.sc_sbcode 과목코드, SB.sb_name 과목명,
        SB.sb_prof 담임교수, SC.sc_score 점수
FROM tbl_score SC
...
-- DECODE(칼럼명, '값', returnn)
--      칼럼명에 '값'이 담겨있으면 return명령을 수행하라
-- 과목명 칼럼에 국어 문자열이 담겨 있으면 해당 레코드의 
--      점수 칼럼 값을 표시하고
--      그렇지 않으면 (null)을 표시하라
SELECT 학번, 
    DECODE(과목명,'국어',점수) AS 국어,
    DECODE(과목명,'영어',점수) AS 영어,
    DECODE(과목명,'수학',점수) AS 수학
FROM view_성적정보
ORDER BY 학번;

-- 제2정규화가 되어 있는 Table에 데이터를
...
(
SELECT 학번,
    SUM(DECODE(과목명,'국어',점수)) AS 국어,
    SUM(DECODE(과목명,'영어',점수)) AS 영어,
    SUM(DECODE(과목명,'수학',점수)) AS 수학,
    SUM(DECODE(과목명,'데이터베이스',점수)) AS DB,
    SUM(DECODE(과목명,'미술',점수)) AS 미술,
    SUM(DECODE(과목명,'음악',점수)) AS 음악,
    SUM(DECODE(과목명,'소프트웨어공학',점수)) AS SW
FROM view_성적정보
GROUP BY 학번
);
