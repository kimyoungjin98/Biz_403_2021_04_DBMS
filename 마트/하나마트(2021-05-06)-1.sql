...
--      매출금액은 VAT가 포함된 금액이다
-- 매입과 매출데이터로 지난 1년간 납부한 VAT 금액을 계산해 보자
-- 매입금액 : 22737397
-- 매출금액 : 41683800
...
SELECT 
        ...
        SUM(DECODE(io_inout,'매출',ROUND(io_total/1.1))) as 매출부가세,
        SUM(DECODE(io_inout,'매출',ROUND(io_total/1.1))) 
        - SUM(DECODE(io_inout,'매입',ROUND(io_total*0.1))) as 납부세액
FROM tbl_iolist
WHERE io_inout = '매출';

-- 20201-01-01 부터 2020-06-30 기간동안
-- 거래된 리스트를 거래처별로 조회

SELECT io_dname as 거래처, io_date as 날짜
FROM tbl_iolist
...
/*
매입 매출 데이터로 부터 상품정보 테이블 데이터를 생성하기
1. 매입매출 데이터에서 상품명으로 그룹지정
2. 매입, 매출 구분에따라 각각 매입단가, 매출단가를 가져오기 
3. 매입과 매출에 0인 값이 있다

4. 매입단가가 0인 데이터는 매출데이터에서 임의로 생성하기
 매출단가의 80%를 매입단가로 하고, 부가세를 제외한 금액으로 계산
 E2항목의 값을 0.8을 곱하여 E2의 80% 가격이 되고
