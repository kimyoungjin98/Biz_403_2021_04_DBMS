...
/*
매입 매출 데이터에서 거래처 정보를 추출하고 
거래처 정보 데이터를 생성한 후
거래처 코드를 만들고 
tbl_dept Table을 작성한 다음 데이터 import 

iolist io_dcode 칼럼을 추가하고 
데이터를 UPDATE 수행
*/
...
-- import된 거래처정보와 매입매출정보를 JOIN 하여 
-- NULL 값이 있는지 확인

SELECT DP.d_code, DP.d_name, DP.d_ceo
FROM tbl_dept DP
    LEFT JOIN tbl_iolist IO
        ON DP.d_name = IO.io_dname AND io_dceo = d_ceo
WHERE DP.d_name IS NULL;
...
SELECT io_date, io_time, io_pcode, p_name,
